...
const makeSut = () => {
  ...
  const sut = new ResetPasswordService(userRepositoryStub, resetPasswordRepositoryStub);
...
describe('ResetPasswordService', () => {
  it('Password updated successfully', async () => {
    ...
    expect(await sut.execute(tokenMock, passwordMock, passwordMock)).toBeUndefined();

    expect(resetPasswordRepositoryStub.getTokenAndLoginByUserId).toHaveBeenCalledWith(
      userId,
    );
    expect(userRepositoryStub.updatePassword).toHaveBeenCalled();
    expect(resetPasswordRepositoryStub.deleteToken).toHaveBeenCalledWith(userId);
  });
