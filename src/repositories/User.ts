...
export class UserRepository extends BaseEntity {
  ...
  async getUserById(
    ...
    { id: UUID; email: string; confirmed: boolean; password_hash: string } | undefined
  > {
    return db('users')
  ...
  async getUserByUsernameOrEmail(
    username: string,
    email: string,
  ): Promise<{ username: string; email: string; confirmed: boolean } | undefined> {
    return db('users')
      ...
      .catch((error) => {
        ...
        this.handlerError.unprocessableEntityError(this.getUserByUsernameOrEmail.name);
      });
  }
  ...
  async getRoleAndPermissions(userId: UUID) {
    return db('users_roles')
      ...
      .catch((error) => {
        ...
        this.handlerError.unprocessableEntityError(this.getRoleAndPermissions.name);
      });
  }
}
